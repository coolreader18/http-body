...
impl<T> Frame<T> {
    ...
    /// # Panics
    ///
    /// If this isn't a DATA frame, this will panic. You should check `is_data`
    /// before calling this.
    // === REVIEW: is this the right method to expose? ===
    pub fn into_data(self) -> T {
        match self.kind {
            Kind::Data(data) => data,
            _ => panic!("called into_data on a non-data frame"),
        }
    }
